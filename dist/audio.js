export class AudioStore {
  ...
  // A tiny synthesized clock tick. It is intentionally generated in the same
  // AudioContext / master gain as the voice and ambient noise, so iPhone treats
  // it as one app-owned output.
  scheduleTick(when = 0, { accent = false } = {}) {
    const context = this.context;
    const output = this.masterGain ?? this.outputNode ?? context?.destination;
    if (!context || context.state !== "running" || !output) return null;

    const startAt = Math.max(context.currentTime + 0.012, Number(when) || 0);
    const duration = accent ? 0.052 : 0.042;
    const peak = accent ? 0.115 : 0.082;

    try {
      const oscillator = context.createOscillator();
      const gain = context.createGain();
      oscillator.type = "triangle";
      oscillator.frequency.setValueAtTime(accent ? 1420 : 1260, startAt);
      oscillator.frequency.exponentialRampToValueAtTime(accent ? 880 : 760, startAt + duration);
