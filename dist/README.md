# CountVoice v39 - live interval and clock tick
...
- 「10秒ごと」では、10・20・30・40・50秒だけでなく、1分・2分・1時間などの単位も読み上げます。
- カウントダウンでは、開始時間が10秒単位でなくても、残り時間がちょうど1分・2分などになった瞬間を読み上げます。


## v39：実行中の間隔切替と毎秒の針音

- カウント中・一時停止中にも、集中画面内の `毎秒 / 10秒ごと` ボタンから読み上げ間隔を切り替えられます。
- `10秒ごと` は、10秒・20秒…と分／時間の節目だけを読み上げます。
- `10秒ごと` の間は、読み上げとは別に時計の針が進むような小さなTickを毎秒鳴らします。Tickは読み上げ・ながし音と同じアプリ内音量に従います。
- 切替時は先の予約音声を破棄し、現在の時点から読み上げ／針音を予約し直します。

---

## v41｜アイコンだけを public/ で管理してGitHub Pagesへ公開

アプリ本体はリポジトリ直下のままです。Safari・PWA用のアイコンだけを `public/` に置いています。

- 開発中の参照先：`./public/apple-touch-icon.png` など
- GitHub Pages公開時：`scripts/build-pages.mjs` が `public/` の中身を成果物のルートへコピー
- 公開URLでは `https://silovar-uk.github.io/countvoice/apple-touch-icon.png` のように見える構成

GitHubで一度だけ **Settings → Pages → Build and deployment → Source → GitHub Actions** を選んでください。以後は `main` へpushすると、`.github/workflows/deploy-pages.yml` が `dist/` を公開します。

Safariのお気に入りを作り直す前に、以下がブラウザで画像として開けることを確認してください。

- `https://silovar-uk.github.io/countvoice/apple-touch-icon.png`
- `https://silovar-uk.github.io/countvoice/favicon.ico`
