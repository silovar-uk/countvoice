...
const TICK_SOUND_INTERVAL_SECONDS = 10;
...
const els = {
  ...
  liveIntervalControl: $("liveIntervalControl"),
  liveIntervalButtons: Array.from(document.querySelectorAll("[data-live-interval]")),
  liveIntervalHint: $("liveIntervalHint"),
  mode: $("mode"),
...
function bindEvents() {
  ...
  for (const button of els.liveIntervalButtons) {
    button.addEventListener("click", () => setLiveIntervalFromUserAction(button.dataset.liveInterval));
  }
  ...
  els.intervalSeconds.addEventListener("change", () => {
    syncLiveIntervalControl();
    rescheduleRunningAudio();
  });
...
function tick() {
  ...
  // 10秒ごとの場合は、読み上げとは別に毎秒の針音をWeb Audioで予約する。
  // 音声パックがないブラウザ音声でも、針音と読み上げを併用できる。
  if (usesAudioGraph()) {
...
async function scheduleAudioAhead() {
  const shouldScheduleVoice = usesBufferedPack();
  const shouldScheduleTicks = usesTickSound();
  if (!state.running || (!shouldScheduleVoice && !shouldScheduleTicks)) return;
  ...
  for (let second = state.scheduledUntilSecond + 1; second <= lastSecond; second += 1) {
    ...
    // 10秒ごと設定では、言葉は節目だけにして、時計の針が進むような
    // 小さなTickを毎秒混ぜる。読み上げ・ながし音と同じ出力へ送る。
    if (shouldScheduleTicks) {
      const tickSource = audioStore.scheduleTick(when, {
        accent: second % TICK_SOUND_INTERVAL_SECONDS === 0,
      });
      if (tickSource) rememberScheduledSource(tickSource);
    }

    if (!shouldScheduleVoice || !shouldSpeak(second, countValue)) continue;
...
function syncLiveIntervalControl() {
  const selected = Number(els.intervalSeconds.value) || 1;
  for (const button of els.liveIntervalButtons) {
    const value = Number(button.dataset.liveInterval);
    const active = value === selected;
    button.setAttribute("aria-pressed", String(active));
    button.dataset.active = String(active);
  }

  if (!els.liveIntervalHint) return;
  if (selected === 10) {
    els.liveIntervalHint.textContent = "毎秒、針音";
  } else if (selected === 1) {
    els.liveIntervalHint.textContent = "毎秒、読み上げ";
  } else {
    els.liveIntervalHint.textContent = `${selected}秒ごと`;
  }
}

async function setLiveIntervalFromUserAction(rawInterval) {
  const next = Number(rawInterval);
  if (![1, 10].includes(next)) return;

  els.intervalSeconds.value = String(next);
  syncLiveIntervalControl();

  if (!state.running) return;

  // 10秒ごとへ切り替えた瞬間は、iPhoneでもタップ起点でTick用の
  // AudioContextを用意する。無音のまま切り替わるのを避ける。
  if (usesAudioGraph() && audioStore.state !== "running") {
    const ready = await prepareAudioForUserAction({ forceRecreate: false });
    if (!ready) return;
  }

  await rescheduleRunningAudio({ clearRecovery: true });
}

async function rescheduleRunningAudio({ clearRecovery = false } = {}) {
  updateBackgroundStatus();
  if (!state.running) return;
  ...
  await rescheduleAudioFromCurrentPosition({ clearRecovery });
}
...
function usesTickSound() {
  return Number(els.intervalSeconds.value) === TICK_SOUND_INTERVAL_SECONDS;
}

function usesAudioGraph() {
  return usesBufferedPack() || state.ambientNoiseMode !== "none" || usesTickSound();
}
